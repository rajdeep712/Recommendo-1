...
def Home_page(request):
    ...
    return render(request, "home/index.html", {"slider_data": slider_data})
...
def fetch_recommendations_from_api(fav_titles, top_n=200):
    if hasattr(fav_titles, "values_list"):
        fav_titles = list(fav_titles.values_list("title", flat=True))
    else:
        fav_titles = list(fav_titles)
    payload = {"fav_movies": fav_titles, "top_k": top_n}
    try:
...
def GetSliderMovies(request):
    ...
    if request.user.is_authenticated and request.user.profile.favourites.first():
        fav_movies = request.user.profile.favourites.all()
        if category == "trending":
            ...
            fav_movie_titles = list(fav_movies.values_list("title", flat=True))
            trendings = fetch_recommendations_from_api(fav_movie_titles)

            order = Case(
                *[When(title=title, then=pos) for pos, title in enumerate(trendings)]
            )
            recommendations = Movie.objects.filter(title__in=trendings).order_by(order)
            ...
            order = Case(
                *[
                    When(code=code, then=pos)
                    for pos, code in enumerate(recommended_codes)
                ]
            )
            recommendations = Movie.objects.filter(code__in=recommended_codes).order_by(
                order
            )
            ...
            movies = (
                Movie.objects.filter(casts__in=fav_casts)
                .distinct()
            ...
            top_rated = Movie.objects.filter(
                Q(imdb_rating__gte=8) & Q(vote_count__gte=1500)
            ).order_by("-popularity")
        ...
        if category == "trending":
            trendings = Movie.objects.filter(Q(vote_count__gte=1000)).order_by(
                "-popularity"
            )
...
def SingleMoviePage(request, code):
    ...
    if request.method == "GET":
        ...
        is_box_office_hit = (
            movie.revenue and movie.budget and movie.revenue >= 2 * movie.budget
        )
        is_blockbuster = (
            movie.revenue
            and movie.budget
            and movie.revenue >= 5 * movie.budget
            and movie.imdb_rating
            and movie.imdb_rating >= 8.0
        )
...
def SearchMovie(request):
    ...
    return render(request, "home/search.html", {"movies": movies})
...
def FilterMovie(request):
    ...
    if category == "All":
        movies = Movie.objects.filter(
            Q(imdb_rating__gt=7.5) & Q(vote_count__gte=1000)
        ).order_by("-popularity")
        ...
        movies = Movie.objects.filter(
            Q(genres__icontains=category) & Q(vote_count__gt=1000)
        ).order_by("-popularity")
...
def SearchResult(request):
    ...
    movies = Movie.objects.filter(title__icontains=mov_name).order_by("-imdb_votes")[
        :600
    ]
    ...
    if not movies:
        ...
        rel_movies = Movie.objects.filter(vote_count__gt=1000).order_by("-popularity")[
            :20
        ]
        mov_data = []
        rel_mov_data = []
...
def AuthFavourites(request):
    if request.user.profile.favourites.first():
        ...
        return render(
            request,
            "home/favs.html",
            {"is_fav": True, "movies": movies, "is_cast": False},
        )
        ...
        return render(
            request,
            "home/favs.html",
            {"is_fav": False, "movies": None, "is_cast": False},
        )
...
def CastMoviesPage(request, name):
    return render(
        request,
        "home/favs.html",
        {"is_fav": True, "movies": None, "is_cast": True, "cast_name": name},
    )
...
def GetCastMovies(request):
    ...
    movies = Movie.objects.filter(casts__name__icontains=cast_name).order_by(
        "-imdb_votes"
    )
...
def ToggleComment(request):
    ...
    if category == "comment":
        ...
        comment_obj = Comment.objects.create(
            name=name, username=username, comment=comment, movies=movie, avatar=avatar
        )
...
def getCasts(request):
    ...
    for cast in casts:
        cast_data.append(
            {"name": cast.name, "id": cast.id, "image_url": cast.image_url}
        )
